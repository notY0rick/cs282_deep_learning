...
def gram_matrix(features, normalize=True):
    ...
    G = torch.tensordot(features, features, dims=([2, 3], [2, 3]))
    if normalize: G /= (H * W * C)
    return G.reshape(N, C, C)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def tv_loss(img, tv_weight):
    ...
    d_vert = vert - img[:, :, :-1, :]
    d_hori = hori - img[:, :, :, :-1]
    return tv_weight * torch.sum(torch.sum(d_vert * d_vert) + torch.sum(d_hori * d_hori))
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
