...
class AttentionQKV(nn.Module):
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        weights =  F.softmax(masked_similarity / th.sqrt(key_dim), dim=-1) # Turn the similarity into a normalized output. Remember that the last dim contains the features
...
class MultiHeadProjection(nn.Module):
    ...
    def _combine_heads(self, tensor):
        ...
        new_feature_size = int(feature_size * self.n_heads) # What is the new feature size, if we combine all the heads
